...
/**
 * Custom action to archive nodes via moderation state.
 *
 * This action leverages the BulkModerationService to transition nodes into
 * the 'archived' state while ensuring all translations are processed.
 */
#[Action(
  id: 'leap_node_archive_action',
  label: new TranslatableMarkup('Archive content'),
  type: 'node'
)]
final class NodeArchiveAction extends EntityActionBase implements ContainerFactoryPluginInterface {

  /**
   * Constructs a NodeArchiveAction object.
   *
  ...
  public function execute(?EntityInterface $entity = NULL): void {
    ...
    $updated_entity = $this->moderationService->transitionNode($entity, 'archived');

    // Verification: Ensure the entity is actually archived (not published).
    if (!$updated_entity || $updated_entity->isPublished()) {
      $this->messenger()->addError($this->t('Something went wrong. The entity could not be archived.'));
    }
  }
